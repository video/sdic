...
(defun sdicf--normalize-max-count (value variable)
  (cond
   ((null value) nil)
   ((and (integerp value) (>= value 0)) value)
   (t
    (error "%s must be nil or non-negative integer: %S" variable value))))

(defun sdicf--max-count-option (value)
  (when (and value (> value 0))
    (list "-m" (number-to-string value))))

(defun sdicf--limit-entries (entries max-count)
  (if (null max-count)
      entries
    (let ((n max-count)
          ret)
      (while (and entries (> n 0))
        (push (car entries) ret)
...
配列の各要素は、
    ...
    strategy を終了する関数
    strategy を使って検索する関数
の5つの要素からなるリストとなっている。strategy の自動判定を行うときは、
...
見つかったエントリのリストを返す。REGEXP が Non-nil の場合は `sdicf-invalid-method' を送出する。"
...
SDIC 辞書オブジェクトは、先頭要素が `SDIC' のベクタで、残りにファイル名・
coding-system・strategy・作業用バッファを持つ、以下の5スロット構造である。
    ・[0] `SDIC' (タグ)
    ・[1] ファイル名
    ・[2] 辞書の coding-system
    ・[3] strategy
...
値によって変化する。指定された検索方式に strategy が対応していない場合
は、sdicf-invalid-method を送出する。"
  (or (sdicf-object-p sdic)
      (signal 'wrong-type-argument (list 'sdicf-object-p sdic)))
  (or (stringp word)
