...
各要素は (バックエンドシンボル ファイルパス (オプション名 値) ...) の形式を取れる。
例: \='((sdicf-client \"/usr/dict/gene.sdic\")
    (sdicf-client \"/usr/dict/gene2.sdic\" (strategy grep)))"
  :type '(repeat (cons (choice (const sdicf-client)
                               (const sdic-compat)
                               (const sdic-gene)
                               symbol)
                       (cons file
                             (repeat (list symbol sexp)))))
  :group 'sdic)
...
例: \='((sdicf-client \"~/data/jedict.sdic\")
    (sdic-compat \"~/data/jgene.dic\" (grep \"/usr/bin/grep\")))"
  :type '(repeat (cons (choice (const sdicf-client)
                               (const sdic-compat)
                               (const sdic-gene)
                               symbol)
...
(defcustom sdic-search-max-count 1000
  "Max count of search results.
検索結果の最大件数を指定する。nil の場合は件数を制限しない。
`sdic.el'、`sdic-compat.el'、`sdicf.el' で共通に参照される。
`sdicf.el' を単体で利用する場合も、この変数を設定する。"
  :type '(choice (const :tag "制限しない" nil) (integer :tag "最大件数"))
  :group 'sdic)
...
(defconst sdic-version
  (eval-when-compile (format-time-string "%Y.%m.%d")))
...
見出し語、辞書シンボル、見出し語のIDからなる配列を要素とする配列を返す。"
  (sdic--ensure-dictionary-open dic)
  (mapcar (lambda (c)
...
見出し語、辞書シンボル、見出し語のIDからなる配列を要素とする配列を返す。
検索時にエラーが発生した辞書はメッセージを表示してスキップする。"
   ...
   (apply #'append
          (mapcar (lambda (dic)
                    (condition-case err
                      ...
                      (error
                       (message "Skip dictionary %s: %s"
                                (sdic--dictionary-label dic)
...
(defun sdic--query-empty-p (query)
  (or (not (stringp query))
      (string= query "")))


(defun sdic--dictionary-label (dic)
  (or (get dic 'title)
      (get dic 'file-name)
      (symbol-name dic)))


(defun sdic--ensure-dictionary-open (dic)
  (or (sdic-open-dictionary dic)
      (error "Can't open dictionary: %s" (sdic--dictionary-label dic))))
...
(defun sdic-describe-word (word &optional search-function)
  ...
  (interactive
   (let ((f (if current-prefix-arg (sdic-select-search-function))))
     (list (sdic-read-from-minibuffer) f)))
  (or (stringp word)
      (signal 'wrong-type-argument (list 'stringp word)))
  (when (sdic--query-empty-p word)
    (user-error "Input word must not be empty"))
  (with-current-buffer (get-buffer-create sdic-buffer-name)
    (or (string= mode-name sdic-mode-name) (sdic-mode))
    (setq buffer-read-only nil)
    (erase-buffer)
    (let ((case-fold-search t))
      (setq sdic-buffer-start-point (point-min))
      (if (prog1
              (funcall (or search-function
...
?               ヘルプ表示"
  ...
  (mapc (lambda (dic)
          (unless (sdic-open-dictionary dic)
            (message "Failed to open dictionary: %s"
                     (sdic--dictionary-label dic))))
        sdic-eiwa-symbol-list)
